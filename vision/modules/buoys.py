...
options = [ options.BoolOption('debug', False),
            ...
            options.IntOption('red_hls_h_min', 98, 0, 255),
            options.IntOption('red_hls_h_max', 255, 0, 255),
            options.IntOption('red_ycrcb_cb_min', 0, 0, 255),
            options.IntOption('red_ycrcb_cb_max', 138, 0, 255),
            options.IntOption('red_lab_a_min', 118, 0, 255),
            options.IntOption('red_lab_a_max', 255, 0, 255),
            options.IntOption('red_lab_b_min', 118, 0, 255),
            ...
            options.IntOption('green_lab_l_min', 137, 0, 255),
            options.IntOption('green_lab_l_max', 255, 0, 255),
            options.IntOption('green_hsv_v_min', 133, 0, 255),
            options.IntOption('green_hsv_v_max', 255, 0, 255),
            options.IntOption('green_hsv_h_min', 69, 0, 255),
            options.IntOption('green_hsv_h_max', 87, 0, 255),
            options.IntOption('green_ycrcb_y_min', 113, 0, 255),
            ...
            options.IntOption('yellow_hls_h_min', 55, 0, 255),
            options.IntOption('yellow_hls_h_max', 87, 0, 255),
            options.IntOption('yellow_ycrcb_cb_min', 0, 0, 255),
            options.IntOption('yellow_ycrcb_cb_max', 132, 0, 255),
            options.IntOption('yellow_lab_a_min', 0, 0, 255),
            options.IntOption('yellow_lab_a_max', 112, 0, 255),
            options.IntOption('yellow_lab_b_min', 122, 0, 255),
            ...
            options.IntOption('max_y_diff', 100, 0, 500) ]
...
options = [ options.BoolOption('debug', False),
            ...
            options.IntOption('min_x_diff', 50, 0, 500)]
...
#TRANSDEC PRELIM
#GREEN_REF = (180, 100, 130)
#YELLOW_REF = (160, 110, 140)
#RED_REF = (140, 130, 120)
        ...
        for buoy in mapping:
            if classification[buoy] is not None:
                for c in coords:
                    ...
                    print(abs(classification[buoy].center[0]-c[0]))
                    if abs(classification[buoy].center[0]-c[0]) < self.options['min_x_diff']:
                        return False
'''
        return True
    ...
    def process(self, mat):
        ...
        gneg = green_morphed.copy()
        rneg = red_morphed.copy()
        yneg = yellow_morphed.copy()

        gneg = cv2.bitwise_not(green_morphed)
        rneg = cv2.bitwise_not(red_morphed)
        yneg = cv2.bitwise_not(yellow_morphed)

        morph_neg = gneg & rneg & yneg

        #total_morphed = green_morphed + (red_morphed + yellow_morphed)
        total_morphed = cv2.bitwise_not(morph_neg)

        if self.options['debug']:
            self.post('morph', total_morphed)

        _, total_contours, _ = cv2.findContours(total_morphed.copy(),
                                              cv2.RETR_EXTERNAL,
                                              cv2.CHAIN_APPROX_SIMPLE)

        contours = []
        contours.extend(total_contours)
